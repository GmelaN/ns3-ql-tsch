    ...
    static TypeId tid =
        TypeId("ns3::LrWpanRadioEnergyModel")
            ...
            .AddTraceSource(
                ...
                "ns3::TracedValueCallback::std::string, std::string, bool, double, double, double"); // TODO: it might be wrong
    return tid;
}
...
void
LrWpanRadioEnergyModel::ChangeLrWpanState(Time t,
...
void
LrWpanRadioEnergyModel::SetLrWpanRadioState(const PhyEnumeration state)
{
    ...
    m_EnergyStateLogger(preStateName,
                        curStateName,
                        m_sourceEnergyUnlimited,
                        m_energyToDecrease,
                        m_remainingBatteryEnergy,
                        m_totalEnergyConsumption);

    NS_LOG_DEBUG("LrWpanRadioEnergyModel:Switching to state: " << curStateName << " at time = "
                                                               << Simulator::Now());
}
