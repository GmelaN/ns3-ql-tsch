...
{
    NS_LOG_FUNCTION(this);
    if (!m_mac || !m_omac || !m_csmaca || !m_phy || !m_node || m_configComplete)
    {
        return;
    }
    ...
    m_mac->SetMcpsDataIndicationCallback(
        MakeCallback(&LrWpanTschNetDevice::McpsDataIndication, this));
    m_omac->SetMcpsDataIndicationCallback(
        MakeCallback(&LrWpanTschNetDevice::McpsDataIndication, this));
    ...
    m_mac->SetMlmeSetSlotframeConfirmCallback(
        MakeCallback(&LrWpanTschNetDevice::SlotframeConfirm, this));
...
LrWpanTschNetDevice::Send(Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
    ...
    if (m_isTsch)
    {
        ...
        FrameControlOptions frmcontrol;
...
bool
LrWpanTschNetDevice::Send(Ptr<Packet> packet,
                          const Address& dest,
                          bool use_ack,
...
bool
LrWpanTschNetDevice::SendFrom(Ptr<Packet> packet,
...
LrWpanTschNetDevice::SetTschMode(bool enable)
{
    ...
    if (m_isTsch != true && enable)
    {
        ...
        m_phy->SetPlmeGetAttributeConfirmCallback(
            MakeCallback(&LrWpanTschMac::PlmeGetAttributeConfirm, m_mac));
        m_phy->SetPlmeSetTRXStateConfirmCallback(
            MakeCallback(&LrWpanTschMac::PlmeSetTRXStateConfirm, m_mac));
        m_phy->SetPlmeSetAttributeConfirmCallback(
            MakeCallback(&LrWpanTschMac::PlmeSetAttributeConfirm, m_mac));
    ...
    if (m_isTsch != false && !enable)
    {
        ...
        m_phy->SetPlmeGetAttributeConfirmCallback(
            MakeCallback(&LrWpanMac::PlmeGetAttributeConfirm, m_omac));
        m_phy->SetPlmeSetTRXStateConfirmCallback(
            MakeCallback(&LrWpanMac::PlmeSetTRXStateConfirm, m_omac));
        m_phy->SetPlmeSetAttributeConfirmCallback(
            MakeCallback(&LrWpanMac::PlmeSetAttributeConfirm, m_omac));
