...
struct MlmeSetLinkRequestParams
{
    ...
    std::bitset<8>
        linkOptions; // b0 = Transmit, b1 = Receive, b2 = Shared, b3= Timekeeping, b4–b7 reserved.
...
typedef Callback<void, uint64_t> MacTimeSlotStartCallback;
...
class LrWpanTschMac : public LrWpanMac
{
   ...
   * \param c the callback
    ...
    void SetMacTimeSlotStartCallback(MacTimeSlotStartCallback c);
   ...
   *  @param energyLevel 0x00-0xff ED level for the channel
     */
    void PlmeEdConfirm(PhyEnumeration status, uint8_t energyLevel) override;
   ...
   *  @param attribute the attribute value
    ...
    void PlmeGetAttributeConfirm(PhyEnumeration status,
                                 ...
                                 Ptr<PhyPibAttributes> attribute) override;
   ...
   *  @param id the attributed identifier
     */
    void PlmeSetAttributeConfirm(PhyEnumeration status, PhyPibAttributeIdentifier id) override;
   ...
   * \param macState indicate BUSY oder IDLE channel condition
     */
    void SetLrWpanMacState(TschMacState macState);
   ...
   * MLME-SET-SLOTFRAME.request primitive is used to add, delete, or modify a slotframe at the MAC sublayer
    ...
    /**
   * The MLME-SET-LINK.request primitive requests to add a new link, or delete or modify an existing link at the MAC sublayer.
     */
   ...
   * \param newState the new state
     */
    void ChangeMacState(TschMacState newState);
   ...
   * Parameters are the old mac state and the new mac state.
     */
    TracedCallback<TschMacState, TschMacState> m_macStateLogger;
   ...
   * See IEEE 802.15.4e-2012 section 5.1.2.6.
    ...
    MacTimeSlotStartCallback m_macTimeSlotStartCallback;

    /**
   * The current state of the MAC layer.
     */
    TschMacState m_macState;

    TschMacState m_lrWpanMacStatePending;
   ...
   * Set MAC state to be TSCH_PKT_WAIT_END if no expected packet received after setted waiting time for transmitted frame (m_macTsRxWait)
    ...
    void SetTxLinkQueue(TxQueueRequestElement* newRequestElement,
...
//    double m_currentFadingBias;
   ...
   * \see class CallBackTraceSource
    ...
    TracedCallback<uint32_t, uint32_t, uint8_t, double> m_macLinkInformation;
};
