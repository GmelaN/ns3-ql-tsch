...
static void
AsciiLrWpanTschMacMaxRetriesSinkWithContext(Ptr<OutputStreamWrapper> stream,
                                            std::string context,
                                            Ptr<const Packet> p)
{
    *stream->GetStream() << "m " << Simulator::Now().GetSeconds() << " " << context << " " << *p
                         << std::endl;
}

static void
AsciiLrWpanTschMacMaxRetriesSinkWithoutContext(Ptr<OutputStreamWrapper> stream, Ptr<const Packet> p)
{
    *stream->GetStream() << "m " << Simulator::Now().GetSeconds() << " " << *p << std::endl;
}

static void
AsciiLrWpanTschMacTransmitSinkWithoutContext(Ptr<OutputStreamWrapper> stream, Ptr<const Packet> p)
{
...
EnergyTraceWithContext(Ptr<OutputStreamWrapper> stream, std::string context, uint32_t psize)
{
 ...
 static void
 ConsumedEnergyTracing(Ptr<OutputStreamWrapper> stream_dev,
                       std::string context_dev,
                       std::string prePhyState,
                       std::string curPhyState,
                       bool sourceEnergyUnlimited,
                       double consumedEnergy,
                       double remainingEnergy,
                       double totalConsumedEnergy)
{
     if (!sourceEnergyUnlimited)
         *stream_dev->GetStream() << Simulator::Now().GetSeconds() << "s: " << context_dev
                                  << " Device state switch to: " << curPhyState
                                  << " from: " << prePhyState
                                  << " Device consumed energy of previous state: " <<
                                  consumedEnergy
                                  << "J" << " Source remaining energy: " << remainingEnergy << "J"
                                  << " Total consumed energy: " << totalConsumedEnergy << "J"
                                  << std::endl;
     else
         *stream_dev->GetStream() << Simulator::Now().GetSeconds() << "s: " << context_dev
                                  << " Device state switch to: " << curPhyState
                                  ...
                                  << " Device consumed energy of previous state: " <<
                                  consumedEnergy
                                  << "J" << " Total consumed energy (Unlimited Source): "
                                  << totalConsumedEnergy << "J" << std::endl;
 }

static void
ReceivedPowerTracing(Ptr<OutputStreamWrapper> stream_rec,
                     uint32_t rxId,
                     uint32_t txId,
                     uint8_t channelNum,
                     ...
                     double fadingBiasPower)
{
    *stream_rec->GetStream() << Simulator::Now().GetSeconds() << "s: " << "Received Node " << rxId
                             << " from Node " << txId << " on Channel: " << (int)channelNum
                             << " with power: " << receivedPower << " dBm"
                             << " containing fading bias: " << fadingBiasPower << " dBm"
                             << " (with original " << receivedPower - fadingBiasPower << " dBm)"
                             << std::endl;
}
...
//             {
//                 //*stream_fadingBias -> GetStream () << "Bias[" << (int)i << "][" << (int)j <<
//                 "]["
//                 //<< (int)k << "] = "
...
std::string
LrWpanTschHelper::LrWpanPhyEnumerationPrinter(PhyEnumeration e)
{
    switch (e)
    {
    ...
    case IEEE_802_15_4_PHY_TRX_START:
        return std::string("TRX_START");
    case IEEE_802_15_4_PHY_TRX_SWITCHING:
        return std::string("TRX_SWITCHING");
    default:
...
std::string
LrWpanTschHelper::LrWpanMacStatePrinter(TschMacState e)
{
    switch (e)
    {
    case TSCH_MAC_IDLE:
...
void
LrWpanTschHelper::EnablePcapInternal(std::string prefix,
                                     Ptr<NetDevice> nd,
                                     bool promiscuous,
                                     bool explicitFilename)
{
    ...
    if (!device)
    {
        NS_LOG_INFO("LrWpanTschHelper::EnablePcapInternal(): Device "
                    << device << " not of type ns3::LrWpanTschNetDevice");
        return;
    }
    ...
    if (promiscuous)
    {
        device->GetOMac()->TraceConnectWithoutContext("PromiscSniffer",
                                                      MakeBoundCallback(&PcapSniffLrWpan, file));
        device->GetNMac()->TraceConnectWithoutContext("PromiscSniffer",
                                                      MakeBoundCallback(&PcapSniffLrWpan, file));
    }
    else
    {
        device->GetOMac()->TraceConnectWithoutContext("Sniffer",
                                                      MakeBoundCallback(&PcapSniffLrWpan, file));
        device->GetNMac()->TraceConnectWithoutContext("Sniffer",
                                                      MakeBoundCallback(&PcapSniffLrWpan, file));
    }
}

void
LrWpanTschHelper::EnableAsciiInternal(Ptr<OutputStreamWrapper> stream,
                                      std::string prefix,
                                      Ptr<NetDevice> nd,
                                      bool explicitFilename)
{
    ...
    if (!device)
    {
        NS_LOG_INFO("LrWpanTschHelper::EnableAsciiInternal(): Device "
                    << device << " not of type ns3::LrWpanTschNetDevice");
        return;
    }
    ...
    if (!stream)
    {
        ...
        asciiTraceHelper.HookDefaultReceiveSinkWithoutContext<LrWpanTschNetDevice>(device,
                                                                                   "MacRx",
                                                                                   theStream);

        device->GetOMac()->TraceConnectWithoutContext(
            "MacTx",
            MakeBoundCallback(&AsciiLrWpanTschMacTransmitSinkWithoutContext, theStream));
        device->GetNMac()->TraceConnectWithoutContext(
            "MacTx",
            MakeBoundCallback(&AsciiLrWpanTschMacTransmitSinkWithoutContext, theStream));
        device->GetOMac()->TraceConnectWithoutContext(
            "MacMaxRetries",
            MakeBoundCallback(&AsciiLrWpanTschMacMaxRetriesSinkWithoutContext, theStream));
        device->GetNMac()->TraceConnectWithoutContext(
            "MacMaxRetries",
            MakeBoundCallback(&AsciiLrWpanTschMacMaxRetriesSinkWithoutContext, theStream));
        asciiTraceHelper.HookDefaultEnqueueSinkWithoutContext<LrWpanTschNetDevice>(device,
                                                                                   "MacTxEnqueue",
                                                                                   theStream);
        asciiTraceHelper.HookDefaultDequeueSinkWithoutContext<LrWpanTschNetDevice>(device,
                                                                                   "MacTxDequeue",
                                                                                   theStream);
        asciiTraceHelper.HookDefaultDropSinkWithoutContext<LrWpanTschNetDevice>(device,
                                                                                "MacTxDrop",
                                                                                theStream);
    ...
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacRx";
    device->GetNMac()->TraceConnect(
        "MacRx",
        oss.str(),
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacTx";
    device->GetNMac()->TraceConnect(
        "MacTx",
        oss.str(),
        MakeBoundCallback(&AsciiLrWpanTschMacTransmitSinkWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacMaxRetries";
    device->GetNMac()->TraceConnect(
        "MacMaxRetries",
        oss.str(),
        MakeBoundCallback(&AsciiLrWpanTschMacMaxRetriesSinkWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacTxEnqueue";
    device->GetNMac()->TraceConnect(
        "MacTxEnqueue",
        oss.str(),
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacTxDequeue";
    device->GetNMac()->TraceConnect(
        "MacTxDequeue",
        oss.str(),
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacTxDrop";
    device->GetNMac()->TraceConnect(
        "MacTxDrop",
        oss.str(),
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacRx";
    device->GetOMac()->TraceConnect(
        "MacRx",
        oss.str(),
        MakeBoundCallback(&AsciiTraceHelper::DefaultReceiveSinkWithContext, stream));
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacTx";
    device->GetOMac()->TraceConnect(
        "MacTx",
        oss.str(),
        MakeBoundCallback(&AsciiLrWpanTschMacTransmitSinkWithContext, stream));
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacMaxRetries";
    device->GetOMac()->TraceConnect(
        "MacMaxRetries",
        oss.str(),
        MakeBoundCallback(&AsciiLrWpanTschMacMaxRetriesSinkWithContext, stream));
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacTxEnqueue";
    device->GetOMac()->TraceConnect(
        "MacTxEnqueue",
        oss.str(),
        MakeBoundCallback(&AsciiTraceHelper::DefaultEnqueueSinkWithContext, stream));
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacTxDequeue";
    device->GetOMac()->TraceConnect(
        "MacTxDequeue",
        oss.str(),
        MakeBoundCallback(&AsciiTraceHelper::DefaultDequeueSinkWithContext, stream));
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacTxDrop";
    device->GetOMac()->TraceConnect(
        "MacTxDrop",
        oss.str(),
        MakeBoundCallback(&AsciiTraceHelper::DefaultDropSinkWithContext, stream));
...
LrWpanTschHelper::EnableEnergyAll(Ptr<OutputStreamWrapper> stream)
{
 ...
 void
 LrWpanTschHelper::EnableEnergyAllPhy(Ptr<OutputStreamWrapper> stream, EnergySourceContainer
 sources)
{
     ...
     for (EnergySourceContainer::Iterator i = sources.Begin(); i != sources.End(); ++i)
     {
         ...
         DeviceEnergyModelContainer devicePerSrcPtr =
             lrWpanSourcePtr->FindDeviceEnergyModels("ns3::LrWpanRadioEnergyModel");
         ...
         for (uint32_t j = 0; j < devicePerSrcPtr.GetN(); j++)
         {
             ...
             Ptr<LrWpanRadioEnergyModel> lrWpanRadioModelPtr =
                 DynamicCast<LrWpanRadioEnergyModel>(devicePerSrcPtr.Get(j));
             NS_ASSERT(lrWpanRadioModelPtr);

             lrWpanRadioModelPtr->TraceConnect("CurrentEnergyState",
                                               oss.str(),
                                               MakeBoundCallback(&ConsumedEnergyTracing, stream));
         }
     }
 }

void
LrWpanTschHelper::EnableReceivePower(Ptr<OutputStreamWrapper> stream_recPower,
    ...
    for (NodeContainer::Iterator i = lrwpanNodes.Begin(); i != lrwpanNodes.End(); ++i)
    {
        ...
        for (uint32_t j = 0; j < node->GetNDevices(); ++j)
        {
            ...
            device->GetPhy()->TraceConnectWithoutContext(
                "PhyLinkInformation",
                MakeCallback(&LrWpanTschMac::GetPhylinkInformation, device->GetNMac()));
            device->GetNMac()->TraceConnectWithoutContext(
                "MacLinkInformation",
                MakeBoundCallback(&ReceivedPowerTracing, stream_recPower));
        }
    }
}
...
void
LrWpanTschHelper::EnableEnergyInternal(Ptr<OutputStreamWrapper> stream,
                                       std::string prefix,
                                       Ptr<NetDevice> nd,
                                       bool explicitFilename)
{
    ...
    if (!device)
    {
        NS_LOG_INFO("LrWpanTschHelper::EnableEnergyInternal(): Device "
                    << device << " not of type ns3::LrWpanTschNetDevice");
        return;
    }
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacRxDataTxAck";
    device->GetNMac()->TraceConnect("MacRxDataTxAck",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacTxData";
    device->GetNMac()->TraceConnect("MacTxData",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacRxData";
    device->GetNMac()->TraceConnect("MacRxData",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacTxDataRxAck";
    device->GetNMac()->TraceConnect("MacTxDataRxAck",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacSleep";
    device->GetNMac()->TraceConnect("MacSleep",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacIdle";
    device->GetNMac()->TraceConnect("MacIdle",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacChannelBusy";
    device->GetNMac()->TraceConnect("MacChannelBusy",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacWaitAck";
    device->GetNMac()->TraceConnect("MacWaitAck",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/TschMac/MacEmptyBuffer";
    device->GetNMac()->TraceConnect("MacEmptyBuffer",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));
    ...
    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacRxDataTxAck";
    device->GetOMac()->TraceConnect("MacRxDataTxAck",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacTxData";
    device->GetOMac()->TraceConnect("MacTxData",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacRxData";
    device->GetOMac()->TraceConnect("MacRxData",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacTxDataRxAck";
    device->GetOMac()->TraceConnect("MacTxDataRxAck",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacSleep";
    device->GetOMac()->TraceConnect("MacSleep",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacIdle";
    device->GetOMac()->TraceConnect("MacIdle",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacChannelBusy";
    device->GetOMac()->TraceConnect("MacChannelBusy",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacWaitAck";
    device->GetOMac()->TraceConnect("MacWaitAck",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));

    oss.str("");
    oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid
        << "/$ns3::LrWpanTschNetDevice/Mac/MacEmptyBuffer";
    device->GetOMac()->TraceConnect("MacEmptyBuffer",
                                    oss.str(),
                                    MakeBoundCallback(&EnergyTraceWithContext, stream));
}
...
void
LrWpanTschHelper::AddLink(Ptr<NetDevice> src,
                          Ptr<NetDevice> dst,
...
void
LrWpanTschHelper::AddLink(NetDeviceContainer devs,
                          u_int32_t srcPos,
                          u_int32_t dstPos,
                          AddLinkParams params,
                          bool sharedLink)
...
LrWpanTschHelper::AddAdvLink(NetDeviceContainer devs, u_int32_t senderPos, AddLinkParams params)
{
    ...
    linkRequest.RxID = 0;
    devs.Get(senderPos)->GetObject<LrWpanTschNetDevice>()->GetNMac()->MlmeSetLinkRequest(
        linkRequest);
    ...
    for (u_int32_t i = 0; i < devs.GetN(); i++)
        if (i != senderPos)
        {
            ...
            linkRequest.TxID = senderPos;
            linkRequest.RxID = i;
            devs.Get(i)->GetObject<LrWpanTschNetDevice>()->GetNMac()->MlmeSetLinkRequest(
                linkRequest);
        }
}

void
LrWpanTschHelper::AddBcastLinks(NetDeviceContainer devs,
                                u_int32_t coordinatorPos,
                                AddLinkParams params)
{
    MlmeSetLinkRequestParams linkRequest;
    linkRequest.Operation = MlmeSetLinkRequestOperation_ADD_LINK;
...
void
LrWpanTschHelper::ModifyLink(NetDeviceContainer devs,
                             u_int32_t srcPos,
                             u_int32_t dstPos,
                             AddLinkParams params,
                             bool sharedLink)
...
void
LrWpanTschHelper::DeleteLink(NetDeviceContainer devs,
                             u_int32_t srcPos,
                             u_int32_t dstPos,
                             AddLinkParams params,
                             bool sharedLink)
...
void
LrWpanTschHelper::ConfigureSlotframeAllToPan(NetDeviceContainer devs,
                                             int empty_timeslots,
                                             bool bidir,
                                             bool bcast)
{
    ...
    */
    ...
    for (u_int32_t i = 0; i < devs.GetN(); i++)
    {
        Simulator::Schedule(Seconds(start),
                            &LrWpanTschNetDevice::SetTschMode,
                            devs.Get(i)->GetObject<LrWpanTschNetDevice>(),
                            true);
...
void
LrWpanTschHelper::GenerateTraffic(Ptr<NetDevice> dev,
                                  Address dst,
                                  int packet_size,
                                  double start,
                                  double duration,
