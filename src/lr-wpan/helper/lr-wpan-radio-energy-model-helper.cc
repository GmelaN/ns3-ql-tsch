...
LrWpanRadioEnergyModelHelper::DoInstall(Ptr<NetDevice> device, Ptr<EnergySource> source) const
{
    ...
    Ptr<LrWpanTschNetDevice> tschdevice = DynamicCast<LrWpanTschNetDevice>(device);
    model->SetEnergyDepletionCallback(
        MakeCallback(&LrWpanPhy::HandleEnergyDepletion, tschdevice->GetPhy()));
    source->AppendDeviceEnergyModel(model);

    // track transceiver state
    Ptr<LrWpanTschNetDevice> lrWpanTschNetDevice = DynamicCast<LrWpanTschNetDevice>(device);
    Ptr<LrWpanPhy> lrWpanPhy = lrWpanTschNetDevice->GetPhy();
    lrWpanPhy->TraceConnectWithoutContext(
        "TrxState",
        MakeCallback(&LrWpanRadioEnergyModel::ChangeLrWpanState, model));
