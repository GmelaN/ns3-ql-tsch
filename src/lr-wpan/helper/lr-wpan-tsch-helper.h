...
class LrWpanTschHelper : public PcapHelperForDevice, public AsciiTraceHelperForDevice
{
  public:
    ...
    /**
     * @brief InstallEnergyDevice: Install LrWpan TSCH DeviceEnergyModel to NetDeviceContainer
     * attached with given energy sources
     ...
     */
    DeviceEnergyModelContainer InstallEnergyDevice(NetDeviceContainer devices,
     ...
     */
    static std::string LrWpanPhyEnumerationPrinter(PhyEnumeration);

    /**
     * @brief LrWpanMacStatePrinter: print link MAC states
     */
    static std::string LrWpanMacStatePrinter(TschMacState e);
     ...
     */
    void AddLink(Ptr<NetDevice> src,
                 Ptr<NetDevice> dst,
                 AddLinkParams params,
                 bool sharedLink = false);
    void AddLink(NetDeviceContainer devs,
                 u_int32_t srcPos,
                 u_int32_t dstPos,
                 AddLinkParams params,
                 bool sharedLink = false);
    ...
    void ModifyLink(NetDeviceContainer devs,
                    u_int32_t srcPos,
                    u_int32_t dstPos,
                    AddLinkParams params,
                    bool sharedLink);
    ...
    void DeleteLink(NetDeviceContainer devs,
                    u_int32_t srcPos,
                    u_int32_t dstPos,
                    AddLinkParams params,
                    bool sharedLink);
     ...
     */
    void ConfigureSlotframeAllToPan(NetDeviceContainer devs,
                                    int empty_timeslots,
                                    bool bidir,
                                    bool bcast);
    ...
    /**
     * @brief EnableEnergyAll: tracing energy for all devices of each node based on MAC timeslot
     * type
    ...
    /**
     * @brief EnableEnergyAll: tracing energy for certain device based on different MAC timeslot
     * types
     * @param stream: output stream to certain file
     * @param dev
     */
    ...
    /**
     * @brief EnableEnergyAllPhy: tracing energy for all devices of each node based on
     different
     * traceiver PHY states
     * @param stream: output stream to certain file
     * @param sources
     */
     ...
     * @param interval
     */
    void GenerateTraffic(Ptr<NetDevice> dev,
                         Address dst,
                         int packet_size,
                         double start,
                         double duration,
    ...
    //    /**
    //     * @brief Set the bias coefficients which describe the multi-path effect on the current
    //     channel
    //     */
    //    void SetFadingBiasValues();

    //    /**
    //     * @brief Print the bias coefficients which describe the multi-path effect on the current
    //     * channel
    //     */
  ...
  private:
    ...
    LrWpanTschHelper& operator=(const LrWpanTschHelper&);
    /**
     * \internal
     *
    ...
    //    LrWpanArray3D<double> FadingBias;    // 3D bias coefficient matrix which describe the
    //    multi-path
    // effect of specific link on specific channel
